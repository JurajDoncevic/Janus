...
public class WrapperCsvFilesTests
{
    ...
    private DataSource GetExpectedInferredDataSetSchemaModel() =>
        SchemaModelBuilder.InitDataSource("DataSet")
            .AddSchema("CarsSchema",
                conf => conf.AddTableau("cars", conf => conf.AddAttribute("Car", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(0))
                            ...
                            .AddTableau("origins", conf => conf.AddAttribute("OriginId", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(0))
                                                               .AddAttribute("FullName", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(1))))
    ...
    private DataSource GetJoinableDataSetSchemaModel() =>
            SchemaModelBuilder.InitDataSource("DataSet")
                .AddSchema("CarsSchema",
                    conf => conf.AddTableau("cars", conf => conf.AddAttribute("Car", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(0))
                                                                .AddAttribute("MPG", conf => conf.WithDataType(DataTypes.DECIMAL).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(1))
                                                                ...
                                                                .AddAttribute("Model", conf => conf.WithDataType(DataTypes.INT).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(7))
                                                                .AddAttribute("Origin", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(8)))
                                .AddTableau("origins", conf => conf.AddAttribute("OriginId", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(false).WithOrdinal(0)) // this is true
                                                                   .AddAttribute("FullName", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(1))))
                .AddSchema("CerealSchema",
                    conf => conf.AddTableau("cereal", conf => conf.AddAttribute("name", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(0))
                                                                  .AddAttribute("mfr", conf => conf.WithDataType(DataTypes.STRING).WithIsPrimaryKey(false).WithIsNullable(true).WithOrdinal(1))
    ...
    public void InferSchemaOnCsvFilesExample()
    {
        ...
        var expectedSchemaModel = GetExpectedInferredDataSetSchemaModel();
    ...
    {
        var dataSource = GetExpectedInferredDataSetSchemaModel();
        ...
        Assert.Equal(2, queryResult.Data?.AttributeNames.Count);
    }

    [Fact(DisplayName = "Run query on DataSet with QueryRunner using join")]
    public async void CsvQueryRunnerJoinTest()
    {
        var dataSource = GetJoinableDataSetSchemaModel();
        var query = QueryModelBuilder.InitQueryOnDataSource("DataSet.CarsSchema.cars", dataSource)
            .WithJoining(conf => conf.AddJoin("DataSet.CarsSchema.cars.Origin", "DataSet.CarsSchema.origins.OriginId"))
            .WithSelection(conf => conf.WithExpression(EQ("DataSet.CarsSchema.cars.Origin", "Europe")))
            .WithProjection(conf => conf.AddAttribute("DataSet.CarsSchema.cars.Car")
                                        .AddAttribute("DataSet.CarsSchema.cars.Origin")
                                        .AddAttribute("DataSet.CarsSchema.origins.OriginId")
                                        .AddAttribute("DataSet.CarsSchema.origins.FullName"))
            .Build();
        ...
        Assert.True(queryResult);
        Assert.Equal(4, queryResult.Data?.AttributeNames.Count);
    }
}