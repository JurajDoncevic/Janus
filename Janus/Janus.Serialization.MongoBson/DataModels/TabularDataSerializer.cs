...
public sealed class TabularDataSerializer : ITabularDataSerializer<byte[]>
{
    ...
    public Result<TabularData> Deserialize(byte[] serialized)
        => Results.AsResult(() => Utils.FromBson<TabularDataDto>(serialized))
            .Bind(FromDto);
        ...
        => Results.AsResult(() =>
        {
            var tabularDataDto = new TabularDataDto
            {
                ...
                AttributeValues = tabularData.RowData
                                             .Select(rd => rd.ColumnValues.ToDictionary(kv => kv.Key, kv => ConvertToBytes(kv.Value, kv.Value?.GetType() ?? typeof(object))))
                                             .ToList()
            };
