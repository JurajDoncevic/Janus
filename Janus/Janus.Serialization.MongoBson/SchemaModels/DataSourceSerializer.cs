...
namespace Janus.Serialization.MongoBson.SchemaModels;
...
public class DataSourceSerializer : IDataSourceSerializer<byte[]>
{
    ...
    public Result<DataSource> Deserialize(byte[] serialized)
        => ResultExtensions.AsResult(() => Utils.FromBson<DataSourceDto>(serialized))
            .Bind(FromDto);
    ...
    public Result<byte[]> Serialize(DataSource dataSource)
        => ResultExtensions.AsResult(()
            => ToDto(dataSource)
                .Map(dataSourceDto => Utils.ToBson(dataSourceDto))
        );
